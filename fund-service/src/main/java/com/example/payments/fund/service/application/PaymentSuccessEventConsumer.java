...
import com.example.payments.fund.service.infrastructure.persistence.PaymentEventConsumptionEntity;
import com.example.payments.fund.service.infrastructure.persistence.PaymentEventConsumptionMapper;
...
@Component
@RocketMQMessageListener(topic = "${fund.payment-success.topic:PAYMENT_SUCCEEDED}",
    consumerGroup = "${fund.payment-success.consumer-group:fund-payment-success}",
    maxReconsumeTimes = 5, consumeMode = ConsumeMode.CONCURRENTLY, messageModel = MessageModel.CLUSTERING,
    consumeTimeout = 60)
public class PaymentSuccessEventConsumer implements RocketMQListener<String> {
  ...
  public PaymentSuccessEventConsumer(LedgerEntryApplicationService ledgerService,
      PaymentEventConsumptionMapper consumptionMapper, ObjectMapper objectMapper) {
  ...
  public void onMessage(String message) {
    ...
    if (schemaVersion != 1) throw new IllegalArgumentException("unsupported payment event schema version");
    ...
    if (record == null) {
      record = createRecord(event, eventId, orderId, merchantId, currency, amount, message, hash, now);
      try {
    ...
    if ("PROCESSED".equals(record.getStatus()) || "DUPLICATE".equals(record.getStatus())) {
      ledgerService.recordPaymentSuccess(idempotencyKey(orderId), orderId, merchantId, amount, currency);
      return;
    }
    if ("FAILED".equals(record.getStatus()) || "REPLAYING".equals(record.getStatus())) {
      if (consumptionMapper.claim(record.getId(), consumerId, now, now.plusSeconds(PROCESSING_LEASE_SECONDS)) != 1) {
    ...
    try {
      var result = ledgerService.recordPaymentSuccess(idempotencyKey(orderId), orderId, merchantId, amount, currency);
      ...
      String failureType = exception instanceof LedgerEntryApplicationService.LedgerConflictException ? "CONFLICT" : "PROCESSING";
      consumptionMapper.markFailed(record.getId(), consumerId, failureType, truncate(exception.getMessage()),
          LocalDateTime.now(ZoneOffset.UTC));
      throw exception;
    }
  }

  private PaymentEventConsumptionEntity createRecord(JsonNode event, String eventId, String orderId,
      String merchantId, String currency, BigDecimal amount, String message, String hash, LocalDateTime now) {
    var record = new PaymentEventConsumptionEntity();
    record.setEventId(eventId); record.setEventType(EVENT_TYPE); record.setOrderId(orderId);
    record.setAttemptId(event.path("attemptId").textValue()); record.setMerchantId(merchantId);
    record.setAmount(amount); record.setCurrency(currency); record.setPayload(message); record.setPayloadHash(hash);
    record.setStatus("PROCESSING"); record.setConsumeCount(1); record.setFirstReceivedAt(now); record.setLastReceivedAt(now);
    record.setProcessingOwner(consumerId); record.setProcessingUntil(now.plusSeconds(PROCESSING_LEASE_SECONDS));
    return record;
  }

  private static void validateExisting(PaymentEventConsumptionEntity record, String eventId, String orderId,
      String merchantId, String currency, BigDecimal amount, String hash) {
    if (!hash.equals(record.getPayloadHash()) || !eventId.equals(record.getEventId()) || !orderId.equals(record.getOrderId())
        || !merchantId.equals(record.getMerchantId()) || !currency.equals(record.getCurrency())
  ...
  private JsonNode parseWithConfiguredMapper(String message) {
    ...
    catch (Exception exception) { throw new IllegalArgumentException("invalid payment success event", exception); }
  }
  ...
  private static String idempotencyKey(String orderId) { return "payment-success:" + orderId; }
