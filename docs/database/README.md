...
本目录是当前后端数据库的完整 SQL 归档。`payment-acquiring-complete.sql` 是唯一的新环境初始化入口，包含表结构、初始化数据、产品管理升级后的产品字段，以及国家/币种基础数据、菜单和权限。应用启动不依赖 Flyway。
...
`payment-acquiring-complete.sql` 必须始终反映当前最新表结构、索引、菜单权限和初始化数据。每次数据库变更都必须在同一变更中同步更新该文件；本目录不再维护拆分的升级、索引或菜单初始化脚本。已有环境升级前应由数据库发布流程基于备份和变更审计执行经过评审的 SQL；新环境直接执行完整 SQL。
