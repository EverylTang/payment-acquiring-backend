...
- 退款申请按订单行锁串行化，幂等键复用时校验金额和原因。
- 退款 Attempt、渠道订单号、处理快照、失败重试和 DEAD 状态。
- 模拟渠道退款回调的时间戳、nonce、签名和 callbackId 去重。
- Fund 退款冲正分录及累计冲正金额上限校验。
- 结算账单摘要导入、自动总额/笔数对账和差异记录。
- 运营后台的 Outbox/DLQ 重发和对账差异处理页面。
- `scripts/run-payment-e2e.sh` 用于已启动 Trade/Fund 服务的 HTTP 验收。
