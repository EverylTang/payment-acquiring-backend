...
import com.example.payments.trade.service.infrastructure.persistence.PaymentCallbackRecordRepository;
...
public class PaymentAttemptService {
  ...
  private final com.example.payments.trade.service.infrastructure.persistence.PaymentOutboxEventRepository outboxRepository;
  private final ObjectMapper objectMapper;

  public PaymentAttemptService(PaymentAttemptRepository repository, PaymentCallbackRecordRepository callbackRepository,
      PaymentChannelAdapter channel, OrderService orderService,
      com.example.payments.trade.service.infrastructure.persistence.PaymentOutboxEventRepository outboxRepository,
  ...
  public PaymentAttempt create(PaymentOrder order, String behavior) {
    ...
    var request = new PaymentChannelAdapter.PaymentChannelRequest(attemptId, order.orderId(), order.merchantId(),
        order.currency(), order.paymentMethod(), order.amount().toPlainString(), behavior);
  ...
  public PaymentAttempt callback(String rawPayload, String signature, String callbackId) {
    var callback = channel.verifyCallback(new PaymentChannelAdapter.PaymentCallbackRequest(rawPayload, signature, callbackId));
    if (!callbackRepository.claim(callbackId, rawPayload, signature, Instant.now())) {
      return repository.findByChannelRequestNo("simulated-channel", callback.channelOrderId())
          .orElseThrow(() -> new ResponseStatusException(HttpStatus.CONFLICT, "duplicate callback"));
    }
    var attempt = repository.findByChannelRequestNo("simulated-channel", callback.channelOrderId())
    ...
    callbackRepository.markProcessed(callbackId, next.attemptId(), callback.channelOrderId(), next.status().name(), Instant.now());
  ...
  public PaymentAttempt query(String attemptId) {
    ...
    var result = channel.queryPayment(new PaymentChannelAdapter.PaymentChannelQuery(attempt.attemptId(), attempt.channelRequestNo()));
  ...
  public PaymentAttempt cancel(String attemptId) {
    ...
    return applyResult(attempt, channel.cancelPayment(new PaymentChannelAdapter.PaymentChannelQuery(attempt.attemptId(), attempt.channelRequestNo())));
  }

  @Transactional
  public PaymentAttempt retry(String attemptId, PaymentOrder order) {
    var previous = repository.findByAttemptId(attemptId)
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "payment attempt not found"));
    if (!(previous.status() == PaymentAttemptStatus.FAILED || previous.status() == PaymentAttemptStatus.TIMEOUT
  ...
  private void coordinateOrder(PaymentAttempt attempt) {
    var orderStatus = switch (attempt.status()) {
  ...
  private static PaymentAttemptStatus statusOf(String value) {
    try { return PaymentAttemptStatus.valueOf(value.toUpperCase()); }
    catch (IllegalArgumentException exception) { return PaymentAttemptStatus.UNKNOWN; }
  }
}
