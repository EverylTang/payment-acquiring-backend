...
public class PaymentAttemptRepository {
  ...
  public PaymentAttempt insert(PaymentAttempt attempt) {
    ...
    entity.setAttemptId(attempt.attemptId());
    entity.setOrderId(attempt.orderId());
    entity.setChannelId(attempt.channelId());
    entity.setChannelRequestNo(attempt.channelRequestNo());
    ...
    return Optional.ofNullable(
            mapper.selectOne(
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<
                    ...
                    .eq(PaymentAttemptEntity::getAttemptId, attemptId)
                    .last("LIMIT 1")))
        .map(this::toDomain);
  }
    ...
    return mapper.findQueryable(toLocal(now), maxQueryCount, limit).stream()
        .map(
            entity -> {
              ...
              boolean claimed =
                  mapper.claimForQuery(
                          entity.getAttemptId(),
                          queryWorkerId,
                          claimToken,
                          toLocal(now),
                          toLocal(now.plusSeconds(lockSeconds)),
                          maxQueryCount)
                      == 1;
              return claimed
                  ? new PaymentAttemptQueryClaim(
                      toDomain(entity), entity.getQueryCount(), claimToken)
                  : null;
            })
  ...
  private PaymentAttempt toDomain(PaymentAttemptEntity e) {
    return new PaymentAttempt(
        ...
        e.getOrderId(),
        e.getChannelId(),
        e.getChannelRequestNo(),
        e.getAttemptNo(),
        PaymentAttemptStatus.valueOf(e.getStatus()),
        e.getRequestSummary(),
        e.getResponseSummary(),
        e.getFailureCode(),
