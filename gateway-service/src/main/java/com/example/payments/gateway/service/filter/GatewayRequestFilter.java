...
public class GatewayRequestFilter implements GlobalFilter, Ordered {
  private static final List<String> HOP_BY_HOP_HEADERS = List.of(
      ...
      HttpHeaders.PROXY_AUTHORIZATION, HttpHeaders.TE, HttpHeaders.TRAILER,
      HttpHeaders.TRANSFER_ENCODING, HttpHeaders.UPGRADE);
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    if (path.startsWith("/api/internal/")
        && (internalToken.isBlank() || !internalToken.equals(request.getHeaders().getFirst("X-Internal-Token")))) {
    ...
    var mutatedRequest = request.mutate().headers(headers -> {
      ...
      if (path.startsWith("/api/admin/") && !internalToken.isBlank()) headers.set("X-Gateway-Token", internalToken);
