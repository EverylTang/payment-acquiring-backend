...
public class MerchantApiAuthenticationService {
  ...
  public String authenticate(AuthenticationRequest request) {
    ...
    if (Math.abs(now.getEpochSecond() - request.timestamp()) > MAX_CLOCK_SKEW_SECONDS) {
      throw new IllegalArgumentException("merchant request timestamp is outside the permitted window");
    }
    var credential =
        credentialMapper.selectActiveApiCredential(request.keyId(), now);
    ...
    String expected = hmac(cipher.decrypt(credential.secretCiphertext()), request.canonicalPayload());
    if (!MessageDigest.isEqual(
        expected.getBytes(StandardCharsets.US_ASCII), request.signature().getBytes(StandardCharsets.US_ASCII))) {
      throw new IllegalArgumentException("merchant authentication failed");
    }
    if (credentialMapper.claimNonce(
            credential.merchantId(), request.keyId(), request.nonce(), now.plusSeconds(MAX_CLOCK_SKEW_SECONDS), now)
        != 1) {
  ...
  private static boolean allowedSource(String sourceIp, String allowlist) {
    ...
    return java.util.Arrays.stream(allowlist.replace("[", "").replace("]", "").replace("\"", "").split(","))
        .map(String::trim)
