...
public class ConfigurationSnapshotService {
  ...
  public ConfigurationSnapshotService(JdbcClient jdbcClient) {
  ...
  public Map<String, Object> snapshot(String merchantId, String productCode, String paymentMethod, String country,
    ...
    var version = jdbcClient.sql("SELECT version_no FROM config_release WHERE status = 'PUBLISHED' ORDER BY version_no DESC LIMIT 1")
    ...
    var product = jdbcClient.sql("SELECT min_amount, max_amount, supports_refund FROM product_capability WHERE product_code = :product AND payment_method = :method AND country = :country AND currency = :currency AND status = 'ACTIVE' AND :amount BETWEEN min_amount AND max_amount")
  ...
  private long count(String sql, long version) {
    return jdbcClient.sql(sql).param("version", version).query(Long.class).single();
  }

  private void requireExists(String sql, String value, String message) {
    if (jdbcClient.sql(sql).param("value", value).query(Long.class).single() == 0) throw unavailable(message);
  }

  private void requireBinding(String merchantId, String productCode) {
    var count = jdbcClient.sql("SELECT COUNT(*) FROM merchant_product WHERE merchant_id = :merchant AND product_code = :product AND status = 'ACTIVE'")
