...
public class AdminAccessController {
  ...
  public AccessResponse current(Authentication authentication) {
    ...
    var roles =
        mybatisClient
            .sql(
                "SELECT r.role_code FROM admin_role r JOIN admin_user_role ur ON ur.role_id = r.id"
                    + " JOIN admin_user u ON u.id = ur.user_id WHERE u.username = :username AND"
                    + " u.status = 'ACTIVE' ORDER BY r.role_code")
    ...
    var menus =
        mybatisClient
            .sql(
                "SELECT DISTINCT m.menu_code, m.menu_name, m.menu_type, m.route_path,"
                    + " m.component_key, m.icon, m.sort_order FROM admin_menu m JOIN"
                    + " admin_role_menu rm ON rm.menu_id = m.id JOIN admin_role r ON r.id ="
                    + " rm.role_id JOIN admin_user_role ur ON ur.role_id = r.id JOIN admin_user u"
                    + " ON u.id = ur.user_id WHERE u.username = :username AND u.status = 'ACTIVE'"
                    + " AND m.status = 'ACTIVE' AND m.visible = TRUE ORDER BY m.sort_order, m.id")
            .param("username", username)
        ...
        mybatisClient
            .sql(
                "SELECT DISTINCT p.permission_code FROM admin_permission p JOIN"
                    ...
                    + " = 'ACTIVE' AND p.status = 'ACTIVE' ORDER BY p.permission_code")
            .param("username", username)
            .query(String.class)
            .list();
