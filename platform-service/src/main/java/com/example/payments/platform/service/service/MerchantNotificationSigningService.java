...
public class MerchantNotificationSigningService {
  ...
  public SignedNotification sign(String merchantId, String body, String nonce, Instant timestamp) {
    var credential = credentialMapper.selectActiveCredentialByMerchantAndType(merchantId, "WEBHOOK", timestamp);
    if (credential == null) throw new IllegalArgumentException("merchant webhook credential is unavailable");
    String canonical = timestamp.getEpochSecond() + "\n" + nonce + "\n" + sha256(body);
    return new SignedNotification(credential.credentialId(), timestamp.getEpochSecond(), nonce, hmac(cipher.decrypt(credential.secretCiphertext()), canonical));
  }
  ...
  private static String hmac(String secret, String value) {
    ...
    } catch (java.security.GeneralSecurityException exception) { throw new IllegalStateException("merchant notification signing failed", exception); }
  }
